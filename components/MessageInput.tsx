...
import { useImageUpload } from "~/utils/handleImage";
import { useFileUpload } from "~/utils/handleFiles";
...
export type Props = {
  ...
  // onShouldSendMedia: (mediaUrl: string, content: string) => void;
};

const MessageInput = forwardRef(
  ({ onShouldSend }: Props, ref) => {
    ...
    useEffect(() => {
      if (storageId) {
        // Use storageId directly as attachId
        // onShouldSendMedia(storageId, "File shared");
        setStorageId(null);
      }
    }, [storageId]); // Only depend on storageId now

    return (
      <Animated.View
        style={[styles.header1, { paddingBottom: bottom, paddingTop: 0 }]}
      >
        {/* <BlurView
              intensity={70}
              tint="dark"
